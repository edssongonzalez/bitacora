  ...
  <div>
    <v-row justify="center">
      <v-col cols="12" xs="12" sm="12" md="12" lg="12" xl="12" class="ml-5 txtsec body-1 text-left mb-5">
        <v-icon color="blue darken-2" left>fa-handshake</v-icon>Tipo de relación de un involucrado en un incidente:
      </v-col>
        ...
        <template>
          ...
          >
            <template v-slot:item.actions="{ item }">
              ...
              <v-icon class="mr-2" @click="estado(item.idtiporel,item.estado)" color="orange darken-3">fa-shuffle</v-icon>
            </template>
                ...
                <v-form v-model="valid" ref="form" lazy-validation @keyup.native.enter="ingresar()">
                  <v-row align="center" justify="center">
                    <v-col cols="12" class="pa-0">
                      <v-text-field dense outlined v-model="tiporel" label="Describa un nombre para la relación" type="text" :rules="minRules" required>></v-text-field>
                    </v-col>
                  </v-row>
                </v-form>
              </v-container>
            ...
            <v-card-text>
              <v-container>
                <v-form v-model="valid" ref="form2" lazy-validation @keyup.native.enter="ingresar()">
                  <v-row align="center" justify="center">
                    <v-col cols="12" class="pa-0">
                      <v-text-field dense outlined v-model="registro.tiporel" label="Describa un nombre para la relación" type="text" :rules="minRules" required>></v-text-field>
                    </v-col>
                  </v-row>
                </v-form>
              </v-container>
...
export default {
  data: () => ({
    ...
    headers: [
      { text: 'Codigo', value: 'idtiporel' },
      { text: 'Nombre', value: 'tiporel' },
        ...
        this.$toastr.confirm('Edición de un registro','¿Confirmar?')
          .then(() => {
            axios.post(process.env.RUTA +'/assets/backend/actualiza.php', {
              accion: 5,
              tiporel:this.registro.tiporel,
              idtiporel:this.registro.idtiporel,
            })
              .then((response)=>{
                if(response.data=='1'){
                  iziToast.success({
