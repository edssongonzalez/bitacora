<template>
  <div class="white">
    <v-card>
      <v-card-title>
        <span class="headline">({{this.id}}) - Fotografias de la persona</span>
        <v-spacer></v-spacer>
...
export default {
  ...
  data() {
    return {
      parametro: null,
      rutafoto:process.env.RUTA +'/assets/backend/foto/',
      dialog: false,
      dfotos:[],
      filesInfo: [],
      theFiles: [],
      ...
      axios.post(process.env.RUTA +'/assets/backend/subir.php', fd, {
        ...
        .then(response => {
          if(response.data=='1'){
            iziToast.success({
              title: 'Fotografia:',
              message: 'relacionada con la persona',
              position: 'topRight',
              timeout: 2000,
            });
            this.cancelar();
