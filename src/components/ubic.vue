  ...
  <div>
    <v-row justify="center">
      <v-col cols="12" xs="12" sm="12" md="12" lg="12" xl="12" class="ml-5 txtsec body-1 text-left mb-5">
        <v-icon color="blue darken-2" left>fa-location</v-icon>Registro de Ubicaciones por Area de Cobertura:
      </v-col>
        ...
        <template>
          ...
          >
            <template v-slot:item.mapa="{ item }">
              <v-icon class="mr-2" color="blue" @click="ver_mapa(item.mapa)">fa-location-dot</v-icon>
            </template>
            <template v-slot:item.actions="{ item }">
              <v-icon class="mr-2" @click="ver_editar(item)" color="orange darken-3">edit</v-icon>
              <v-icon class="mr-2" @click="estado(item.idubicacion,item.estado)" color="orange darken-3">fa-shuffle</v-icon>
            </template>
                ...
                <v-form v-model="valid" ref="form" lazy-validation @keyup.native.enter="ingresar()">
                  <v-row align="center" justify="center">
                    <v-col cols="12" class="pa-0">
                      <v-autocomplete dense outlined append-icon="fa-list" :items="area" v-model="idarea" label="Seleccione el Area" :rules="selRules" required></v-autocomplete>
                    </v-col>
                    <v-col cols="12" class="pa-0">
                      <v-text-field dense outlined v-model="ubicacion" label="Escriba el nombre de la ubicacion" type="text" :rules="minRules" required>></v-text-field>
                    </v-col>
                    <v-col cols="12" class="pa-0">
                      <v-text-field dense outlined v-model="latitud" label="Latitud de la ubicacion" type="text" :rules="minRules" required>></v-text-field>
                    </v-col>
                    <v-col cols="12" class="pa-0">
                      <v-text-field dense outlined v-model="longitud" label="Longitud de la ubicacion" type="text" :rules="minRules" required>></v-text-field>
                    </v-col>
                  </v-row>
                </v-form>
              </v-container>
            ...
            <v-card-text>
              <v-container>
                <v-form v-model="valid" ref="form2" lazy-validation @keyup.native.enter="ingresar()">
                  <v-row align="center" justify="center">
                    <v-col cols="12" class="pa-0">
                      <v-autocomplete dense outlined append-icon="fa-list" :items="area" v-model="registro.idarea" label="Seleccione el Area" :rules="selRules" required></v-autocomplete>
                    </v-col>
                    <v-col cols="12" class="pa-0">
                      <v-text-field dense outlined v-model="registro.ubicacion" label="Escriba el nombre de la ubicacion" type="text" :rules="minRules" required>></v-text-field>
                    </v-col>
                    <v-col cols="12" class="pa-0">
                      <v-text-field dense outlined v-model="registro.latitud" label="Latitud de la ubicacion" type="text" :rules="minRules" required>></v-text-field>
                    </v-col>
                    <v-col cols="12" class="pa-0">
                      <v-text-field dense outlined v-model="registro.longitud" label="Longitud de la ubicacion" type="text" :rules="minRules" required>></v-text-field>
                    </v-col>
                  </v-row>
                </v-form>
              </v-container>
